...
def chassis_plot(start: Posture,
    ...
    def v_in_b(t) -> Velocity:
        ...
        return Velocity(
            ...
            vy=v.vx * math.cos(theta) - v.vy * math.sin(theta),
            wz=v.wz
        )
...
if __name__ == "__main__":
    chassis_plot(
        ...
        end=Posture(x=3, y=1, yaw=180),
        start_velocity=Velocity(vx=0, vy=0, wz=0),
        start_accel=Acceleration(ax=0, ay=0, ayaw=0),
        x_limit=Limit(v_max=5, a_max=1, j_max=2),
        y_limit=Limit(v_max=5, a_max=0.4, j_max=2),
        yaw_limit=Limit(v_max=180, a_max=50, j_max=90)
    )
